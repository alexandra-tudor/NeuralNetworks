...
import cifar10
from utils import plot_confusion_matrix, plot_images_err, make_data
...
def train_network(data, train_step, cost, accuracy, num_iterations, train_batch_size, dropout, placeholders, saver, save_dir, plot_dir, log_dir, figname_suffix):
    with tf.Session() as session:
        ...
        for i in range(total_iterations,
            ...
            if i % 10 == 0:
                ...
                print(msg.format(i + 1, c, train_acc, test_acc))
        ...
        plt.savefig(plot_dir + "train_accuracy_" + figname_suffix + ".png")
        plt.clf()
        plt.plot(*zip(*testing_acc))
        plt.savefig(plot_dir + "test_accuracy_" + figname_suffix + ".png")
        plt.clf()
        plt.plot(*zip(*training_cost))
        plt.savefig(plot_dir + "cost_" + figname_suffix + ".png")
...
def test_network(test_batch_size, placeholders, dropout, saver, save_dir, accuracy, y_true_cls, y_pred_cls, data):
    with tf.Session() as session:
        session.run(tf.global_variables_initializer())
...
def print_results(cls_pred, cls_true, data, img_size, num_channels, fig_name_suffix, plot_dir):
    if num_channels == 3:
        img_shape = (img_size, img_size, num_channels)
    else:
        img_shape = (img_size, img_size)

    plot_images_err(output_dir=plot_dir,
                    fig_name="example_errors" + "_" + fig_name_suffix,
                    img_shape=img_shape,
...
def run_experiment(params, dropout, bn):
    ...
    cls_true, cls_pred = test_network(params['test_batch_size'],
                                      placeholders,
                                      1.0,
                                      saver,
                                      params['save_dir'],
    ...
    return train_acc, cost, test_acc, figname_suffix


if __name__ == "__main__":
    base_dir = '/home/ardelaxela/Research/projects/NeuralNetworks/data-mnist/'
    ...
    stats = []
    stats += [run_experiment(params, False, False)]
    stats += [run_experiment(params, True, False)]
    stats += [run_experiment(params, False, True)]
    stats += [run_experiment(params, True, True)]

    for s in stats:
        plt.plot(*zip(*s[0]), label=s[3])
    plt.legend(loc='best')
