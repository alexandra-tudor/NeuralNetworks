...
def test_network(test_batch_size, placeholders, dropout, saver, save_dir, accuracy, y_true_cls, y_pred_tensor, y_pred_cls, data):
    with tf.Session() as session:
        session.run(tf.global_variables_initializer())
        ...
        while i < num_iterations:
            ...
            y_pred_value, batch_y_pred, batch_y_true, batch_acc = session.run([y_pred_tensor, y_pred_cls, y_true_cls, accuracy], feed_dict=feed_dict_test)
...
if __name__ == "__main__":
    datasets = [('mnist', 28, 1, 1000, 30, 5e-3, 100), ('cifar10', 32, 3, 5000, 100, 5e-3, 100)]
