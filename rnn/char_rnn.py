...
class model(object):
    ...
    def __init__(self, FLAGS):
        ...
        with tf.variable_scope('rnn_inputs'):
            W_input = tf.get_variable("W_input", [FLAGS.NUM_CLASSES, FLAGS.NUM_HIDDEN_UNITS])
        ...
        # <seq_len*BATCH_SIZE, num_hidden_units>
        outputs = tf.reshape(tf.concat(outputs, 1), [-1, FLAGS.NUM_HIDDEN_UNITS])
        ...
        self.loss = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=self.logits, labels=y_as_list))
...
def train(FLAGS):
    ...
    with tf.Session() as sess:
        ...
        for epoch_num, epoch in enumerate(generate_epochs(FLAGS,
            ...
            print("{0} Training loss {1}"
                  "".format(epoch_num, np.mean(train_loss)))

            # Save checkpoint every epoch.
            if not os.path.isdir(FLAGS.CKPT_DIR):
                os.makedirs(FLAGS.CKPT_DIR)
            checkpoint_path = os.path.join(FLAGS.CKPT_DIR, "model.ckpt")
            # print("Saving the model at Epoch {0}.".format(epoch_num))
